    ...
    public static List<ProductToSend> getParsing(JSONArray listShopping)
    {
        ...
        try {
            for(int j=0;j<listShopping.length();j++)
            {
                ...
                p=new ProductToSend(0,name,Integer.parseInt(initQuantity),unit,Float.parseFloat(product_positionx),Float.parseFloat(product_positiony),round(Float.parseFloat(price), 2),false);
                lit.add(p);
            }
